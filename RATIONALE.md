...
The lookup id reveals nothing, and a scan reaches only a few heights around the scanner. That is right for a private message and useless for anything meant to be found. In a 2^255 space, unhinted chalk is chalk nobody walks past.

The hint (spec §7.6) leaves the choice with the hider: an aligned box, one height per axis, published on the bag. The gap between the box and the bag's own region is the price of the search, because the seeker sweeps the box by deriving keys at the bag's height, and the work equivalence of §7.1 makes that sweep cost the same from anywhere. Reading has no distance term. Reaching does: standing in the region with a movement chain to prove it. The protocol separates the two on purpose. The hider gets a difficulty knob. Applications get a clean fact to reward. No registry, no referee, no trusted index.

Per-axis heights come free with aligned geometry, and they do work. Exact X with coarse Y and Z is a two-dimensional hunt. A sector on all three axes says where to go, not where to look. Three heights equal to the bag's height is a destination. One tag covers all of it, and the sector tags of §10 follow whenever a hint fixes a sector, so a relay answers "what is hidden in this sector" with the filters it already has.
