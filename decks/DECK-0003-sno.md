...
**`colors` changed meaning inside `v: 2`, once, and that will not happen again.** On 2026-09-16 `colors` went from three numbers per vertex to one palette index (§1.3), which by the rule above should have made a `v: 3`. It did not, and the reason is stated here rather than hidden: three objects existed in the world at that moment, all published by the authors of this document, and all of them were reissued the same day. The rule protects readers, and there were none. A third encoding would have cost every implementer forever to spare two people an afternoon.
